...
/*
CustomerFXMLController is associated with customerFXML which bacisally developed for serching and updating.
There are two panes on this controller, the first one is search pane and another is update pane. 
Update pane is only active when a entry on list view is slected and update button is pressed.
*/
...
// below are the FXML elements used in the Customer FXML page.
    ...
    private void updateCustomer(ActionEvent event) {
        ...
        try {
            ...
            if (Fname.isEmpty() || Lname.isEmpty() || Address.isEmpty() || Pnumberupdatetxt.getText().isEmpty()) {
                displayMessage("Please fill-up each and every detail !");
            } else {
